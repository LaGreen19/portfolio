<?php
	$selectOptionData = array(
		...
		array(
			'value'	=> 'youtube',
			'text'	=> __('YouTube', OTW_PML_TRANSLATION)
		),
		array(
			'value'	=> 'vimeo',
			'text'	=> __('Vimeo', OTW_PML_TRANSLATION)
		),
		array(
			'value'	=> 'soundcloud',
			'text'	=> __('Sound Cloud', OTW_PML_TRANSLATION)
		),
		array(
			'value'	=> 'img',
			'text'	=> __('Image', OTW_PML_TRANSLATION)
		),
		array(
			'value'	=> 'slider',
			'text'	=> __('Slider', OTW_PML_TRANSLATION)
		)
	);
	...
	// ( !empty( $otw_pm_meta_data['video_url'] ) )? $video_url = $otw_pm_meta_data['video_url'] : $video_url = '';
	// ( !empty( $otw_pm_meta_data['media_url'] ) )? $media_url = $otw_pm_meta_data['media_url'] : $media_url = '';
...
<table class="form-table">
	<tbody>
		<!-- Select Drop Down -->
		<tr valign="top">
			<th scope="row"><label for="media_type"><?php _e('Choose Media Type', OTW_PML_TRANSLATION);?></label></th>
			<td>
		...
		<tr valign="top" class="js-meta-youtube <?php echo $youtube;?>">
			<th scope="row"><label for="youtube_url"><?php _e('Enter YouTube URL', OTW_PML_TRANSLATION);?></label></th>
			<td>
				<input type="text" id="youtube_url" name="otw-pm-list-youtube_url" class="js-otw-youtube-url" value="<?php echo $youtube_url; ?>" size="53"/>
			</td>
		</tr>
		<!-- YouTube URL -->
		...
		<tr valign="top" class="js-meta-vimeo <?php echo $vimeo;?>">
			<th scope="row"><label for="vimeo_url"><?php _e('Enter Viemo URL', OTW_PML_TRANSLATION);?></label></th>
			<td>
				<input type="text" id="vimeo_url" name="otw-pm-list-vimeo_url" class="js-otw-vimeo-url" value="<?php echo $vimeo_url; ?>" size="53"/>
			</td>
		</tr>
		<!-- Vimeo URL -->
		...
		<tr valign="top" class="js-meta-soundcloud <?php echo $soundcloud;?>">
			<th scope="row"><label for="soundcloud_url"><?php _e('Enter SoundCloud URL', OTW_PML_TRANSLATION);?></label></th>
			<td>
				<input type="text" id="soundcloud_url" name="otw-pm-list-soundcloud_url" class="js-otw-soundcloud-url" value="<?php echo $soundcloud_url; ?>" size="53"/>
			</td>
		</tr>
		<!-- ScoundCloud URL -->

<!-- 		<tr valign="top" class="js-meta-media <?php echo $media_display;?>">
			<th scope="row"><label for="media_url"><?php _e('Select Video File', OTW_PML_TRANSLATION);?></label></th>
			<td>
				<input type="hidden" id="media_url" name="otw-pm-list-media_url" class="js-otw-media-url" value="<?php echo $media_url; ?>" size="53"/>
				<a href="#" class="js-add-media"><?php _e('Select Audio File', OTW_PML_TRANSLATION);?></a>
				<p class="description"><?php _e('Note: At this time WordPress will support only MP4 files for native embededing', OTW_PML_TRANSLATION);?></p>
			</td>
		</tr> -->

		<!-- Single Image -->
		<tr valign="top" class="js-meta-image <?php echo $img;?>">
			<th scope="row"><label for="img_upload"><?php _e('Select File', OTW_PML_TRANSLATION);?></label></th>
			<td>
				<a href="#" class="js-add-image"><?php _e('Add File', OTW_PML_TRANSLATION);?></a>
				<input type="hidden" name="otw-pm-list-img_url" class="js-img-url" value="<?php echo $img_url; ?>" />
		...
		<tr valign="top" class="js-meta-slider <?php echo $slider;?>">
			<th scope="row"><label for="slider"><?php _e('Slider Images', OTW_PML_TRANSLATION);?></label></th>
			<td>
				<a href="#" class="js-add-image left"><?php _e('Add File', OTW_PML_TRANSLATION);?></a><br/>
				<input type="hidden" name="otw-pm-list-slider_url" class="js-img-slider-url" value="<?php echo $slider_url; ?>" size="53"/>
